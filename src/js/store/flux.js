	...
	return {
		...
		actions: {
			...
			createContactList: async () => {
				...
				await fetch(
					"https://playground.4geeks.com/contact/agendas/GioEscalona", { 
						method: "POST",
					}
				);
				actions.getContacts(); 
			...
			postContact: async (inputName, inputPhone, inputEmail, inputAddress) => {
				const actions = getActions();
				const response = await fetch("https://playground.4geeks.com/contact/agendas/GioEscalona/contacts", {
					method: "POST",
					body: JSON.stringify({
						name: inputName,
						phone: inputPhone,
			...
			deleteContact: async (id) => {
				const actions = getActions();
				const response = await fetch('https://playground.4geeks.com/contact/agendas/GioEscalona/contacts/' + `${id}`, {
					method: "DELETE",
				})
				if (!response.ok) {
